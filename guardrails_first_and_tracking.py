...
async def main():
    ...
    print("\nProcessing business document query with guardrails first approach...")
    
    try:
        # First check if the query is business-related using the guardrail
        guardrail_check_result = await Runner.run(
            guardrail_agent, 
            document_query,
            run_config=run_config
        )
        
        guardrail_output = guardrail_check_result.final_output_as(BusinessQueryClassification)
        
        if not guardrail_output.is_business_related:
            print("Query rejected: Not business-related")
            print(f"Reasoning: {guardrail_output.reasoning}")
        else:
            print("Query accepted: Business-related")
            print(f"Reasoning: {guardrail_output.reasoning}")
            
            # If query is business-related, pass it to the delegation agent
    ...
    print("\nProcessing business web search query with guardrails first approach...")
    ...
    # Test with a non-business query (should be rejected by guardrail)
    non_business_query = "What's the recipe for chocolate chip cookies?"
    print("\nProcessing non-business query with guardrails first approach...")
